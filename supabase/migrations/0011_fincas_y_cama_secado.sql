-- Catálogo de fincas (mismo patrón que los demás catálogos simples de M1)
...
create policy "lectura autenticados" on public.fincas for select
  using (auth.role() = 'authenticated');
create policy "escritura admin y operador" on public.fincas for insert
  with check ((auth.jwt() ->> 'user_role') in ('admin', 'operador'));
create policy "actualizacion admin y operador" on public.fincas for update
  using ((auth.jwt() ->> 'user_role') in ('admin', 'operador'));
create policy "eliminar solo admin" on public.fincas for delete
  using ((auth.jwt() ->> 'user_role') = 'admin');

create trigger trg_audit_fincas after insert or update or delete on public.fincas
  for each row execute function public.fn_audit_trigger();

-- Lotes: finca de origen + número de cama de secado. Ambos se conservan a
-- través de las transformaciones, igual que variedad/proveedor/nombre.
alter table public.lotes add column finca_id uuid references public.fincas(id);
alter table public.lotes add column numero_cama_secado text;
...
begin
  ...
  insert into public.lotes (nombre, variedad_id, proveedor_id, finca_id, numero_cama_secado, etapa, peso_actual_kg, peso_inicial_kg, fecha_cosecha, proceso_beneficiado_id)
  values (v_origen.nombre, v_origen.variedad_id, v_origen.proveedor_id, v_origen.finca_id, v_origen.numero_cama_secado, v_etapa_destino, v_peso_destino, v_peso_destino, v_origen.fecha_cosecha, v_proceso_id)
...
begin
  ...
  insert into public.lotes (nombre, variedad_id, proveedor_id, finca_id, numero_cama_secado, etapa, peso_actual_kg, peso_inicial_kg, fecha_cosecha, proceso_beneficiado_id)
  values (v_origen.nombre, v_origen.variedad_id, v_origen.proveedor_id, v_origen.finca_id, v_origen.numero_cama_secado, 'verde', v_peso_destino, v_peso_destino, v_origen.fecha_cosecha, v_origen.proceso_beneficiado_id)
  returning id into v_lote_destino_id;
  ...
  loop
    if v_item.peso_kg is not null and v_item.peso_kg > 0 then
      declare
        v_lote_destino_id uuid;
      begin
        insert into public.lotes (nombre, variedad_id, proveedor_id, finca_id, numero_cama_secado, etapa, peso_actual_kg, peso_inicial_kg, fecha_cosecha, proceso_beneficiado_id, calidad)
        values (v_origen.nombre, v_origen.variedad_id, v_origen.proveedor_id, v_origen.finca_id, v_origen.numero_cama_secado, 'verde', v_item.peso_kg, v_item.peso_kg, v_origen.fecha_cosecha, v_origen.proceso_beneficiado_id, v_item.calidad)
        returning id into v_lote_destino_id;
