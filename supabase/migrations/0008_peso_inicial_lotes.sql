-- peso_actual_kg queda en 0 cuando el lote se consume (se transforma o se
-- clasifica) — correcto para el cálculo de stock, pero borra la información
-- de "con cuánto entró" que se necesita para el historial/trazabilidad.
-- peso_inicial_kg se fija una sola vez al crear el lote y nunca cambia.
alter table public.lotes add column peso_inicial_kg numeric;
update public.lotes set peso_inicial_kg = peso_actual_kg where peso_inicial_kg is null;
alter table public.lotes alter column peso_inicial_kg set not null;

-- Los inserts existentes (registrarLote, y las 3 funciones de transformación)
-- se actualizan en el código de la aplicación / esta misma migración para
-- fijar peso_inicial_kg = peso_actual_kg en cada creación.
...
begin
  ...
  insert into public.lotes (variedad_id, proveedor_id, etapa, peso_actual_kg, peso_inicial_kg, fecha_cosecha, proceso_beneficiado_id)
  values (v_origen.variedad_id, v_origen.proveedor_id, v_etapa_destino, v_peso_destino, v_peso_destino, v_origen.fecha_cosecha, v_proceso_id)
...
begin
  ...
  insert into public.lotes (variedad_id, proveedor_id, etapa, peso_actual_kg, peso_inicial_kg, fecha_cosecha, proceso_beneficiado_id)
  values (v_origen.variedad_id, v_origen.proveedor_id, 'verde', v_peso_destino, v_peso_destino, v_origen.fecha_cosecha, v_origen.proceso_beneficiado_id)
  returning id into v_lote_destino_id;
  ...
  loop
    if v_item.peso_kg is not null and v_item.peso_kg > 0 then
      declare
        v_lote_destino_id uuid;
      begin
        insert into public.lotes (variedad_id, proveedor_id, etapa, peso_actual_kg, peso_inicial_kg, fecha_cosecha, proceso_beneficiado_id, calidad)
        values (v_origen.variedad_id, v_origen.proveedor_id, 'verde', v_item.peso_kg, v_item.peso_kg, v_origen.fecha_cosecha, v_origen.proceso_beneficiado_id, v_item.calidad)
        returning id into v_lote_destino_id;
